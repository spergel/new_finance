...
from models import SecurityData, ConversionTerms, MetricsSummary, OriginalDataReference, QuantitativeMetrics, FormulaComponents, StructuredProductMetrics, ConversionMetrics, VWAPMetrics, MinMaxFormulaComponents, AntiDilutionMetrics, FloatingRateMetrics, CashlessExerciseMetrics
...
"""
    ...
    def _create_exotic_conversion_security(self, conversion_data: Dict, ticker: str, filing_type: str) -> Optional[SecurityData]:
        ...
        security = SecurityData(
            id=f"{ticker}_{filing_type}_exotic_conv_{int(datetime.now().timestamp())}",
            company=ticker,
            xbrl_concept='ExoticConvertibleSecurities',
            description=conversion_data.get('security_description', 'Exotic convertible security'),
            cusip=conversion_data.get('cusip', 'N/A'),
            trading_symbol=conversion_data.get('trading_symbol', 'N/A'),
            principal_amount=self._extract_single_amount(conversion_data.get('principal_amount')),
            outstanding_amount=self._extract_single_amount(conversion_data.get('outstanding_amount')),
            interest_rate=self._extract_single_percentage(conversion_data.get('interest_rate')),
            issue_date=self._extract_single_date(conversion_data.get('issue_date')),
            maturity_date=self._extract_single_date(conversion_data.get('maturity_date')),
            filing_date=date.today(),
            liquidation_preference=self._extract_single_amount(conversion_data.get('liquidation_preference')),
            liquidation_terms=conversion_data.get('liquidation_terms'),
            conversion_terms=conversion_terms,
            financial_metrics=MetricsSummary(
                principal_amount=self._extract_single_amount(conversion_data.get('principal_amount')),
    ...
    def _create_trigger_events_security(self, trigger_data: Dict, ticker: str, filing_type: str) -> Optional[SecurityData]:
        ...
        security = SecurityData(
            id=f"{ticker}_{filing_type}_trigger_{int(datetime.now().timestamp())}",
            company=ticker,
            type='TriggerEventSecurities',  # Custom XBRL concept for trigger features
            filing_date=datetime.now().date(),
            description=trigger_data.get('security_description', 'Trigger Event Security'),
            filing_source=filing_type,
            metrics=metrics,
            quantitative_metrics=quantitative_metrics,
            original_data_reference=original_ref,
            llm_commentary=f"TRIGGER EVENT: {trigger_event_details}"
        )
    ...
    def _create_exotic_payments_security(self, payment_data: Dict, ticker: str, filing_type: str) -> Optional[SecurityData]:
        ...
        security = SecurityData(
            ...
            company=ticker,
            type='ExoticPaymentSecurities',  # Custom XBRL concept for exotic payment features
            filing_date=date.today(),
            principal_amount=None,
            rate=None,
            maturity_date=None,
    ...
    def _create_performance_milestones_security(self, milestone_data: Dict, ticker: str, filing_type: str) -> Optional[SecurityData]:
        ...
        security = SecurityData(
            ...
            company=ticker,
            type='PerformanceMilestoneSecurities',  # Custom XBRL concept for performance milestones
            filing_date=date.today(),
            principal_amount=None,
            rate=None,
            maturity_date=None,
            issue_date=None,
            description=milestone_data.get('security_description', 'Security with performance milestone provisions'),
            shares_outstanding=None,
            filing_source=filing_type,
            conversion_terms=None,
            liquidation_terms=None,
            ...
            liquidation_preference=None,
            original_data_reference=original_ref,
            llm_commentary=f"PERFORMANCE MILESTONE: {performance_milestone_details}"
        )
    ...
    def _create_credit_regulatory_triggers_security(self, trigger_data: Dict, ticker: str, filing_type: str) -> Optional[SecurityData]:
        ...
        security = SecurityData(
            ...
            company=ticker,
            type='CreditRegulatoryTriggerSecurities',  # Custom XBRL concept for credit/regulatory triggers
            filing_date=date.today(),
            principal_amount=None,
            rate=None,
            maturity_date=None,
            issue_date=None,
            description=trigger_data.get('security_description', 'Security with credit/regulatory trigger provisions'),
            shares_outstanding=None,
            filing_source=filing_type,
            conversion_terms=None,
            liquidation_terms=None,
            ...
            liquidation_preference=None,
            original_data_reference=original_ref,
            llm_commentary=f"CREDIT/REGULATORY TRIGGER: {credit_regulatory_trigger_details}"
        )
