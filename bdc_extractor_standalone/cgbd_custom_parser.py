#!/usr/bin/env python3
"""
Custom CGBD (TCG BDC Inc) Investment Extractor

CGBD uses XBRL primarily with HTML table enhancement for dates and rates.
"""
...
class CGBDCustomExtractor:
    """Custom extractor for CGBD that uses XBRL with HTML enhancement."""
    ...
    def extract_from_ticker(self, ticker: str = "CGBD"), year: Optional[int] = 2025, min_date: Optional[str] = None) -> Dict:
        """Extract investments from CGBD's latest 10-Q filing."""
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for CGBD")
        ...
        return self.extract_from_filing(txt_url, htm_url, "TCG BDC Inc", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "CGBD") -> Dict:
        """Extract complete CGBD investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting CGBD extraction from XBRL...")
        ...
        output_file = os.path.join(output_dir, f'{ticker}_TCG_BDC_Inc_investments.csv')
...
def main():
    ...
    extractor = CGBDCustomExtractor()
    try:
        result = extractor.extract_from_ticker("CGBD")
