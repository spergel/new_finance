...
OBDC uses XBRL primarily with HTML table enhancement for dates and rates.
"""
...
class OBDCCustomExtractor:
    """Custom extractor for OBDC that uses XBRL with HTML enhancement."""
    ...
    def extract_from_ticker(self, ticker: str = "OBDC") -> Dict:
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for OBDC")
        ...
        return self.extract_from_filing(txt_url, htm_url, "Blue Owl Capital Corp", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "OBDC") -> Dict:
        """Extract complete OBDC investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting OBDC extraction from XBRL...")
