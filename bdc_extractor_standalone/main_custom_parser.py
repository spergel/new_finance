#!/usr/bin/env python3
"""
Custom MAIN (Main Street Capital Corporation) Investment Extractor

MAIN uses XBRL primarily with HTML table enhancement for dates and rates.
"""
...
class MAINCustomExtractor:
    """Custom extractor for MAIN that uses XBRL with HTML enhancement."""
    ...
    def extract_from_ticker(self, ticker: str = "MAIN") -> Dict:
        """Extract investments from MAIN's latest 10-Q filing."""
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for MAIN")
        ...
        return self.extract_from_filing(txt_url, htm_url, "Main Street Capital Corp", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "MAIN") -> Dict:
        """Extract complete MAIN investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting MAIN extraction from XBRL...")
        ...
        output_file = os.path.join(output_dir, f'{ticker}_Main_Street_Capital_Corp_investments.csv')
