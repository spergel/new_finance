...
class GAINExtractor:
    ...
    def extract_from_ticker(self, ticker: str = "GAIN") -> Dict:
        ...
        return self.extract_from_url(txt_url, "Gladstone_Investment_Corp", cik)
    ...
    def extract_from_url(self, filing_url: str, company_name: str, cik: str) -> Dict:
        ...
        out_file = os.path.join(out_dir, 'GAIN_Gladstone_Investment_Corp_investments.csv')
    ...
    def _build_investment(self, context: Dict, facts: List[Dict]) -> Optional[GAINInvestment]:
        if context['company_name']=='Unknown': return None
        inv=GAINInvestment(company_name=context['company_name'],investment_type=context['investment_type'],industry=context['industry'],context_ref=context['id'])
        for f in facts:
...
def main():
    ...
    ex=GAINExtractor()
    try:
        res=ex.extract_from_ticker('GAIN')
