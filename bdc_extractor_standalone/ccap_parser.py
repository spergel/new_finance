...
class CCAPExtractor:
    ...
    def extract_from_ticker(self, ticker: str = "CCAP", year: Optional[int] = 2025, min_date: Optional[str] = None) -> Dict:
    ...
    def _parse_identifier(self, identifier: str) -> Dict[str, str]:
        ...
        for pattern in inv_type_patterns:
            ...
            if match:
                ...
                elif 'Unsecured Debt' in inv_type_text:
                    res['investment_type'] = 'Unsecured Debt'
                elif 'Preferred Stock' in inv_type_text or 'Preferred Equity' in inv_type_text:
                    res['investment_type'] = 'Preferred Equity'
                elif 'Limited Partnership Interest' in inv_type_text:
                    res['investment_type'] = 'Limited Partnership Interest'
                else:
                    res['investment_type'] = inv_type_text
                break
        
        # If still not found, try to find investment type patterns directly (for cases like "Company Investment Type Unsecured Debt")
        if res['investment_type'] == 'Unknown':
            direct_patterns = [
                r'\bUnsecured\s+Debt\b',
                r'\bSecured\s+Debt\b',
                r'\bFirst\s+Lien\b',
                r'\bSecond\s+Lien\b',
                ...
                r'\bCommon\s+Equity\b',
                r'\bLimited\s+Partnership\s+Interest\b',
            ]
            for pattern in direct_patterns:
                match = re.search(pattern, after_industry, re.IGNORECASE)
                if match:
                    res['investment_type'] = match.group(0).strip()
                    break
    ...
    def _extract_html_fallback(self, filing_url: str, investments: List[CCAPInvestment]) -> Optional[Dict[str, Dict]]:
    ...
    def _merge_html_data(self, investments: List[CCAPInvestment], html_data: Dict[str, Dict]):
