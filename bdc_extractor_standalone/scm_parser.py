...
class SCMExtractor:
    ...
    def extract_from_ticker(self, ticker: str = "SCM") -> Dict:
        ...
        return self.extract_from_url(txt_url, "Stellus_Capital_Investment_Corp", cik)
    ...
    def _build_investment(self, context: Dict, facts: List[Dict]) -> Optional[SCMInvestment]:
        if context['company_name']=='Unknown': return None
        inv=SCMInvestment(company_name=context['company_name'],investment_type=context['investment_type'],industry=context['industry'],context_ref=context['id'])
        for f in facts:
