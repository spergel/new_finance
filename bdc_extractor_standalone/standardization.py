...
INVESTMENT_TYPE_MAPPINGS: List[Tuple[str, str]] = [
    ...
    # Already standardized forms - check these first to avoid re-standardizing
    (r'^First\s+Lien\s+Debt\s+-\s+Revolver\s*$', 'First Lien Debt - Revolver'),
    (r'^First\s+Lien\s+Debt\s+-\s+Delayed\s+Draw\s*$', 'First Lien Debt - Delayed Draw'),
    ...
    (r'^First\s+Lien\s+Debt\s*$', 'First Lien Debt'),  # Only match standalone "First Lien Debt"
    (r'First\s+Lien\s+Senior\s+Secured\s+Notes?', 'First Lien Debt'),
    # Generic "Secured Debt" - assume First Lien if no other specification
    (r'^Secured\s+Debt\s*$', 'First Lien Debt'),
    (r'Senior\s+Secured\s+First\s+Lien\s+Term\s+Loan', 'First Lien Debt'),
    # Senior Secured Notes variations with first lien
    (r'Senior\s+Secured\s+Notes?\s+-\s+first\s+lien\s+senior\s+secured\s+notes?', 'First Lien Debt'),
    (r'Senior\s+Secured\s+Notes?\s+-\s+first\s+lien\s+first\s+out\s+senior\s+secured\s+notes?', 'First Lien Debt'),
    (r'Senior\s+Secured\s+Notes?\s+-\s+first\s+lien\s+second\s+out\s+senior\s+secured\s+notes?', 'First Lien Debt'),
    (r'Senior\s+Secured\s+Notes?\s+-\s+first\s+lien\s+third\s+out\s+senior\s+secured\s+notes?', 'First Lien Debt'),
    (r'Senior\s+Secured\s+Notes?\s+-\s+first\s+lien', 'First Lien Debt'),
    # Generic "Senior Secured Notes" - assume First Lien if no other specification
    (r'^Senior\s+Secured\s+Notes?\s*$', 'First Lien Debt'),
    ...
    (r'Senior\s+subordinated\s+loan', 'Subordinated Debt'),
    (r'Senior\s+Subordinated\s+Loan', 'Subordinated Debt'),
    ...
    (r'Class\s+[A-Z]\s+membership\s+units?', 'Common Equity'),
    (r'Class\s+[A-Z]\s+Membership\s+Units?', 'Common Equity'),
...
INDUSTRY_MAPPINGS: List[Tuple[str, str]] = [
    ...
    (r'^Connectivity\s*$', 'Telecommunications'),  # Connectivity often refers to telecom
    ...
    # Healthcare Providers & Services variations
    (r'Healthcare\s+Providers\s*&\s*Services', 'Healthcare Providers & Services'),
    (r'Health\s+Care\s+Providers\s+Services', 'Healthcare Providers & Services'),
    (r'Health\s+Care\s+Providers\s*&\s*Services', 'Healthcare Providers & Services'),
    ...
    (r'Commercial\s+Services\s+Supplies', 'Business Services'),
    ...
    (r'^Capital\s+Goods\s*$', 'Capital Equipment'),
    ...
    (r'Hotels,\s*Restaurants\s*&\s*Leisure', 'Leisure Products & Services'),
    ...
    (r'Food\s+Products', 'Food & Beverage'),
...
REFERENCE_RATE_MAPPINGS: List[Tuple[str, str]] = [
    # SOFR variations - most specific first
    (r'^SOFR\s*$', 'SOFR'),
    (r'^SOFR\s*\([QMS\d]+\)\s*$', 'SOFR'),  # SOFR (Q), SOFR (M), SOFR (S), SOFR (3M), etc.
    (r'Secured\s+Overnight\s+Financing\s+Rate', 'SOFR'),
    (r'SecuredOvernightFinancingRate', 'SOFR'),  # No spaces
    (r'^SF\s*$', 'SOFR'),  # Abbreviation
    ...
    # CDOR / CORRA (Canadian rates)
    (r'^CDOR\s*$', 'CDOR'),
    (r'Canadian\s+Dollar\s+Offered\s+Rate', 'CDOR'),
    (r'^C\s*$', 'CDOR'),  # In formulas
    (r'^CA\s*$', 'CDOR'),  # Canadian abbreviation
    (r'^CORRA\s*\([QMS\d]+\)\s*$', 'CORRA'),  # CORRA (Q), etc.
    (r'^CORRA\s*$', 'CORRA'),
    
    # BASE RATE
    (r'^BASE\s+RATE\s*$', 'BASE RATE'),
    (r'Base\s+Rate', 'BASE RATE'),
    (r'BaseRate', 'BASE RATE'),  # No space
