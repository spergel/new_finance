...
class OXSQExtractor:
    ...
    def _parse_identifier(self, identifier: str) -> Dict[str,str]:
        ...
        if len(parts) >= 3:
            ...
            industry = parts[1]
            # Company name is the rest (parts 2+), but might have additional details
            company_parts = parts[2:]
            
            # Check if last part is investment type details (like "first lien senior secured notes")
            if len(company_parts) > 1:
                ...
                if any(keyword in last_part for keyword in ['first lien', 'second lien', 'senior secured', 'lien']):
                    ...
                    if 'first lien' in last_part and 'first lien' not in investment_type.lower():
                        investment_type = f"{investment_type} - {company_parts[-1]}"
                else:
                    company = ' - '.join(company_parts)
            else:
                company = company_parts[0] if company_parts else 'Unknown'
        elif len(parts) == 2:
            # Format: Investment Type - Industry (aggregate, no company)
            investment_type = parts[0]
            industry = parts[1]
        ...
        res['investment_type'] = investment_type if investment_type else 'Unknown'
