...
OCSL uses XBRL primarily with HTML table enhancement for dates and rates.
"""
...
class OCSLCustomExtractor:
    """Custom extractor for OCSL that uses XBRL with HTML enhancement."""
    ...
    def extract_from_ticker(self, ticker: str = "OCSL") -> Dict:
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for OCSL")
        ...
        return self.extract_from_filing(txt_url, htm_url, "Oaktree Specialty Lending Corp", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "OCSL") -> Dict:
        """Extract complete OCSL investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting OCSL extraction from XBRL...")
        ...
        output_file = os.path.join(output_dir, f'{ticker}_Oaktree_Specialty_Lending_Corp_investments.csv')
