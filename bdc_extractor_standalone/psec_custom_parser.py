#!/usr/bin/env python3
"""
Custom PSEC (Prospect Capital Corp) Investment Extractor

PSEC uses XBRL primarily with HTML table enhancement for dates and rates.
"""
...
class PSECTypedMemberExtractor(TypedMemberExtractor):
    """PSEC-specific XBRL extractor with custom identifier parsing."""
    
    def _parse_investment_identifier(self, identifier: str):
        """
        PSEC-specific identifier parsing.
        
        Formats seen:
        - "Company, Industry, Investment Type"
        - "Company, Investment Type"
        - "CP Energy Services Inc., Energy Equipment & Services, First Lien Term Loan"
        - "Belnick, LLC (d/b/a The Ubique Group), Household Durables, First Lien Term Loan"
        """
        if not identifier:
        ...
        # Investment type patterns (order matters - most specific first)
        ...
        # Industry indicators
        industry_indicators = [
            'household', 'durables', 'energy', 'equipment', 'services',
            'healthcare', 'technology', 'financial', 'consumer',
            'industrial', 'retail', 'manufacturing', 'aerospace',
            'defense', 'pharmaceuticals', 'media', 'telecommunications',
            'data processing', 'outsourced', 'storage', 'transportation',
            'gas', 'oil', 'multi-sector', 'metal', 'glass', 'plastic',
            'containers'
        ]
        ...
        # Handle "Company, Industry, Investment Type" format
        if len(parts) >= 3:
            # Check if last part is investment type
            last_part = parts[-1]
            last_lower = last_part.lower()
            
            for pattern in investment_type_patterns:
                pattern_lower = pattern.lower()
                if pattern_lower in last_lower or last_lower.startswith(pattern_lower):
                    # Last part is investment type
                    investment_type = last_part
                    # Check if second-to-last is industry
                    second_last = parts[-2]
                    if any(ind in second_last.lower() for ind in industry_indicators):
                        # Second-to-last is industry, company is everything before
                        company_name = ','.join(parts[:-2]).strip()
                    else:
                        # Company is everything before last
                        company_name = ','.join(parts[:-1]).strip()
                    return company_name, investment_type
        
        # Handle "Company, Investment Type" format
        if len(parts) == 2:
            ...
            for pattern in investment_type_patterns:
                pattern_lower = pattern.lower()
                if pattern_lower in potential_type.lower() or potential_type.lower().startswith(pattern_lower):
                    return potential_company, potential_type
...
class PSECCustomExtractor:
    """Custom extractor for PSEC that uses XBRL with HTML enhancement."""
    
    def __init__(self, user_agent: str = "BDC-Extractor/1.0 contact@example.com"):
        self.headers = {'User-Agent': user_agent}
        self.sec_client = SECAPIClient(user_agent=user_agent)
        self.xbrl_extractor = PSECTypedMemberExtractor(user_agent)
        self.html_parser = FlexibleTableParser(user_agent=user_agent)
    
    def extract_from_ticker(self, ticker: str = "PSEC") -> Dict:
        """Extract investments from PSEC's latest 10-Q filing."""
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for PSEC")
        ...
        return self.extract_from_filing(txt_url, htm_url, "Prospect Capital Corp", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "PSEC") -> Dict:
        """Extract complete PSEC investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting PSEC extraction from XBRL...")
        ...
        output_file = os.path.join(output_dir, f'{ticker}_Prospect_Capital_Corp_investments.csv')
