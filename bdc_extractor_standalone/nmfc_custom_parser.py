...
class NMFCCustomExtractor:
    ...
    def extract_from_ticker(self, ticker: str = "NMFC") -> Dict:
        ...
        return self.extract_from_filing(txt_url, htm_url, "New Mountain Finance Corp", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "NMFC") -> Dict:
        """Extract complete NMFC investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting NMFC extraction from XBRL...")
        ...
        output_file = os.path.join(output_dir, f'{ticker}_New_Mountain_Finance_Corp_investments.csv')
...
def main():
    ...
    extractor = NMFCCustomExtractor()
    try:
        result = extractor.extract_from_ticker("NMFC")
