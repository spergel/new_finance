...
NCDL uses XBRL primarily with HTML table enhancement for dates and rates.
"""
...
class NCDLCustomExtractor:
    """Custom extractor for NCDL that uses XBRL with HTML enhancement."""
    ...
    def extract_from_ticker(self, ticker: str = "NCDL"), year: Optional[int] = 2025, min_date: Optional[str] = None) -> Dict:
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for NCDL")
        ...
        return self.extract_from_filing(txt_url, htm_url, "Nuveen Churchill Direct Lending Corp", cik, ticker)
    
    def extract_from_filing(self, txt_url: str, htm_url: str, company_name: str, cik: str, ticker: str = "NCDL") -> Dict:
        """Extract complete NCDL investment data from XBRL (primary) and HTML (enhancement)."""
        
        logger.info(f"Starting NCDL extraction from XBRL...")
        ...
        output_file = os.path.join(output_dir, f'{ticker}_Nuveen_Churchill_Direct_Lending_Corp_investments.csv')
