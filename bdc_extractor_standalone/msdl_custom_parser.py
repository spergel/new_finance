...
MSDL uses XBRL primarily for investment data. Rewritten to use direct XBRL extraction.
"""
...
class MSDLCustomExtractor:
    """Custom extractor for MSDL that uses XBRL."""
    
    def __init__(self, user_agent: str = "BDC-Extractor/1.0 contact@example.com"):
        self.headers = {'User-Agent': user_agent}
        self.sec_client = SECAPIClient(user_agent=user_agent)
    
    def extract_from_ticker(self, ticker: str = "MSDL", year: Optional[int] = 2025, min_date: Optional[str] = None) -> Dict:
        ...
        if not match:
            raise ValueError("Could not parse accession/URLs for MSDL")
        
        accession = match.group(1)
        accession_no_hyphens = accession.replace('-', '')
        txt_url = f"https://www.sec.gov/Archives/edgar/data/{cik}/{accession_no_hyphens}/{accession}.txt"
        
        return self.extract_from_url(txt_url, "Morgan_Stanley_Direct_Lending_Fund", cik)
    
    def extract_from_url(self, filing_url: str, company_name: str, cik: str) -> Dict:
        """Extract complete MSDL investment data from XBRL."""
        ...
        investments: List[MSDLInvestment] = []
        for ctx in contexts:
        ...
        out_file = os.path.join(out_dir, 'MSDL_Morgan_Stanley_Direct_Lending_Fund_investments.csv')
    ...
    def _build_investment(self, context: Dict, facts: List[Dict]) -> Optional[MSDLInvestment]:
...
def main():
    ...
    ex=MSDLCustomExtractor()
    try:
        res=ex.extract_from_ticker('MSDL')
